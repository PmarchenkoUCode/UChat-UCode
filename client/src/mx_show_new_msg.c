...
static GtkWidget *create_new_msg_row(t_client *client, t_msgs *cur_msg) {
    ...
    if (client->self->uid == cur_msg->from_uid) {
        ...
        gtk_box_pack_start(GTK_BOX(msg_outer_box), empty_box, TRUE, TRUE, 1);
        gtk_box_pack_end(GTK_BOX(msg_outer_box), msg_box, TRUE, TRUE, 1);
    }
    else {
        sprintf(sender, "%s:", client->selected_chat->peer_name);
        gtk_box_pack_start(GTK_BOX(msg_outer_box), msg_box, TRUE, TRUE, 1);
        gtk_box_pack_end(GTK_BOX(msg_outer_box), empty_box, TRUE, TRUE, 1);
    }
    ...
    gtk_box_pack_start(GTK_BOX(msg_box), sender_name, TRUE, TRUE, 0);
    gtk_box_pack_start(GTK_BOX(msg_box), msg, TRUE, TRUE, 0);
